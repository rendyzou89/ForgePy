...
- Implement component definitions through `BaseComponent`, immutable `ComponentMetadata` and `ComponentManifest`, and validated `ComponentContext`; keep component and metadata names aligned, and register explicitly through `ComponentRegistry.register()`. Manifest file declarations must remain project-relative. A new registry must remain empty by default and must not resolve manifest relationships or perform installation. Do not add built-ins or application integration without an explicit requirement.
