...
- `components/` owns independent component metadata, its validated project context, the component installation hook, and in-memory registration. It has no CLI, template, configuration, builder, or project-generation responsibility.
...
- Implement component definitions through `BaseComponent`, immutable `ComponentMetadata`, and validated `ComponentContext`; keep component and metadata names aligned, and register explicitly through `ComponentRegistry.register()`. A new registry must remain empty by default and must not perform installation. Do not add built-ins or application integration without an explicit requirement.
