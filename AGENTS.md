...
- `components/` owns independent component metadata and in-memory registration. It has no installation, CLI, template, configuration, builder, or project-generation responsibility.
...
- Implement component definitions through `BaseComponent` and immutable `ComponentMetadata`, keep their names aligned, and register explicitly through `ComponentRegistry.register()`. A new registry must remain empty by default. Do not add built-ins, installation behavior, or application integration without an explicit requirement.
