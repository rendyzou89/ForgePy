...
- **Current development area:** Sprint 9.0, introducing an independent empty component-registry foundation without application integration.
...
- Define immutable component metadata, an abstract identity/metadata contract, and an empty in-memory registry with explicit registration, lookup, and ordered listing.
...
The component foundation has no built-ins and is not consumed by the CLI, configuration system, templates, builders, `ProjectGenerator`, or generated projects. It performs no installation.
...
| `components/` | Immutable component metadata, the minimal component contract, and an empty in-memory registry. |
...
- `ComponentRegistry` is intentionally empty and in-memory. It currently provides no discovery, persistence, installation, CLI command, template association, or generation integration.
...
- Finish and verify the Sprint 9.0 component-registry foundation without connecting it to application behavior.
...
| Active development area | Sprint 9.0 component registry foundation |
...
| Component state | Empty independent registry; no built-ins, installation, or application integration |
| Version source | `config/version.py`, aligned with the `v0.6.0` release tag |
| Verification state | `unittest` covers component and template metadata/registries, configuration, create precedence, list output, template architecture and output snapshots, all structures, CLI execution, and isolated template-aware VS Code generation; the real external lifecycle remains a manual check |
