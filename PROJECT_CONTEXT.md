...
- **Current development area:** Sprint 8.1 template metadata, building a multi-template foundation while retaining the single `basic` template.
...
- Register the single `basic` project template through a validated registration path with immutable metadata for its name, description, template version, author, and tags.
...
- Test the configuration store, configuration CLI, create-input precedence, template metadata validation, successful and rejected registration, registry lookup, and list output with `unittest`.
...
| `templates/` | Basic template, immutable metadata, registry, file mapping, rendered content, and VS Code JSON. |
| `utils/` | Present but currently contains no implemented utility behavior. |
| `tests/` | Standard-library automated tests for configuration, create-input resolution, template metadata, registry behavior, and list output. |
...
- ForgePy itself has no declared third-party runtime dependencies. Automated coverage currently focuses on user configuration, its CLI command, create-input resolution, template metadata, registry behavior, and list output. The generated `basic` project requires `PySide6`, `pandas`, and `openpyxl`, so the full create lifecycle attempts package installation and may require network access.
- Template lookup uses direct dictionary indexing; an unknown template is not converted into a friendly CLI error.
- Template metadata versioning has no release policy yet. The current `basic` metadata records `0.6.0` independently from the ForgePy application version and generated-project version.
...
- Finish and verify the Sprint 8.1 metadata contract without changing `basic` generation behavior.
- Keep metadata registration extensible while adding no further templates until separately approved.
...
| Active development area | Sprint 8.1 template metadata and registry foundation |
| Implemented CLI | `create`, `list`, `version`, `config show/set/reset`, plus no-command interactive create |
| Implemented templates | `basic` only, with descriptive metadata |
| Version source | `config/version.py`, aligned with the `v0.6.0` release tag |
| Verification state | `unittest` covers configuration, create-input precedence, template metadata, registry lookup, and list output; use documented checks for the generator lifecycle |
